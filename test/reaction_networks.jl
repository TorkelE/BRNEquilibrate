...
#A ;arge number of reaction networks are added to test as many different cases as possible.
...
fixed_conc = Vector{Vector{Expr}}(undef,10)
...
reaction_networks_fixed_conc[1] = @reaction_network begin
    (k1,k2), X1 ↔ X2
    (k3,k4), X2 ↔ X3
    (k5,k6), X3 ↔ X1
end k1 k2 k3 k4 k5 k6 FC
fixed_conc[1] = [:(X1+X2+X3=FC)]

reaction_networks_fixed_conc[2] = @reaction_network begin
    (k1,k2), X1 ↔ 2X1
    (k3,k4), X1 + X2 ↔ X3
    (k5,k6), X3 ↔ X2
end k1 k2 k3 k4 k5 k6 FC
fixed_conc[2] = [:(X2+X3=FC)]

reaction_networks_fixed_conc[3] = @reaction_network begin
    (k1,k2*X5), X1 ↔ X2
    (k3*X5,k4), X3 ↔ X4
    (p+k5*X2*X3,d), ∅ ↔ X5
end k1 k2 k3 k4 p k5 d FC1 FC2
fixed_conc[3] = [:(X1+X2=FC1),:(X3+X4=FC2)]

reaction_networks_fixed_conc[4] = @reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
    (mm(X3,v,K),d), ∅ ↔ X4
end k1 k2 v K d FC1 FC2
fixed_conc[4] = [:(X1+X3=FC1),:(X2+X3=FC2)]

reaction_networks_fixed_conc[5] = @reaction_network begin
    (k1,k2), X1 ↔ 2X2
    (k3,k4), 2X2 ↔ 3X3
    (k5,k6), 3X3 ↔ 4X4
end k1 k2 k3 k4 k5 k6 FC
fixed_conc[5] = [:(24X1+12X2+8X3+6X4=FC)]

reaction_networks_fixed_conc[6] = @reaction_network begin
    mmR(X1,v1,K1), X1 → X2
    mmR(X2,v2,K2), X2 → X3
    mmR(X3,v3,K3), X3 → X1
end v1 K1 v2 K2 v3 K3 FC
fixed_conc[6] = [:(X1+X2+X3=FC)]

reaction_networks_fixed_conc[7] = @reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
    (mm(X3,v,K),d), ∅ ↔ X2
    (k3,k4), X2 ↔ X4
end k1 k2 k3 k4 v K d FC
fixed_conc[7] = [:(X1+X3=FC)]

reaction_networks_fixed_conc[8] = @reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
    (mm(X3,v1,K1),mm(X4,v2,K2)), X3 ↔ X4
end k1 k2 v1 K1 v2 K2 FC1 FC2
fixed_conc[8] = [:(X1+X2+X4=FC1),:(X2+X3+X4=FC2)]

reaction_networks_fixed_conc[9] = @reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
    (k3,k4), X3 + X4 ↔ X5
    (k5,k6), X5 + X6 ↔ X7
end k1 k2 k3 k4 k5 k6 FC1 FC2 FC3 FC4
fixed_conc[9] = [:(X1+X3+X5+X7=FC1),:(X2+X3+X5+X7=FC2),:(X4+X5+X7=FC3),:(X6+X7=FC4)]

reaction_networks_fixed_conc[10] = @reaction_network rnType begin
